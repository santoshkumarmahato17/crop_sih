...
async def get_current_user(
    ...
    import re as _re
    _demo_match = _re.match(r'^(?:demo-token-|token-)(farmer|admin|government|expert|officer|extension_worker)(.*)$', token.lower())
    if _demo_match or token.startswith('demo-token') or token.startswith('token-'):
        raw_role = _demo_match.group(1).upper() if _demo_match else "GOVERNMENT"
        if raw_role in ["EXPERT", "OFFICER", "EXTENSION_WORKER"]:
