...
class OrangeDiseaseModelService:
    _instance = None

    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(OrangeDiseaseModelService, cls).__new__(cls)
            cls._instance._init_model()
        return cls._instance

    def _init_model(self):
        """Loads the ORANGE_ML_model.keras into memory on startup."""
        self.is_ready = False
        self.model = None
        self.class_names = {}
        ...
        try:
            ...
            model_dir = os.path.join(base_dir, "models", "orange")
            model_path = os.path.join(model_dir, "ORANGE_ML_model.keras")
            class_names_path = os.path.join(model_dir, "class_names.json")

            if not os.path.exists(model_path):
                logger.error(f"Orange model not found at {model_path}")
                return
            ...
            logger.info(f"Loading Orange ML model from {model_path}...")
            self.model = tf.keras.models.load_model(model_path, compile=False)
            self.is_ready = True
            logger.info("Orange ML model loaded successfully.")

        except ImportError:
            logger.error("TensorFlow is not installed. Orange model cannot be loaded.")
        except Exception as e:
            logger.error(f"Failed to load Orange ML model: {e}")
    ...
    def predict(self, image_bytes: bytes) -> Dict[str, Any]:
        ...
        if not self.is_ready or self.model is None:
            raise RuntimeError("Orange model is not loaded or not ready.")
        ...
        high_threshold = float(os.getenv("ORANGE_HIGH_CONFIDENCE_THRESHOLD", "0.80"))
        review_threshold = float(os.getenv("ORANGE_REVIEW_THRESHOLD", "0.60"))
        ...
        return {
            "prediction": {
                ...
                "message": "Orange disease prediction successful." if status != "REVIEW_REQUIRED" else "Prediction confidence is low. Expert review required."
            },
    ...
    async def analyze_image(self, file: UploadFile) -> Dict[str, Any]:
        ...
        if not self.is_ready:
            raise HTTPException(
                status_code=503,
                detail="Orange model is not loaded. Ensure ORANGE_ML_model.keras is present.",
            )
        ...
        try:
            ...
            logger.error(f"Orange inference failed: {e}")
            raise HTTPException(status_code=500, detail=f"Orange inference failed: {str(e)}")
