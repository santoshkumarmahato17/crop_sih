"""
Production Training Pipeline for 7-Class Maize Pest & Disease Classification:
...
from ml.src.dataset_maize import MaizeLeafDataset, CANONICAL_MAIZE_CLASSES, CLASS_CATEGORIES
...
def compute_class_weights(train_samples: List[Dict], num_classes: int) -> torch.Tensor:
    ...
    print(f"[Maize Training] Class counts in training set: {counts}")
    print(f"[Maize Training] Computed class weights: {[round(w, 3) for w in weights_tensor.tolist()]}")
...
def train_maize_model(
    ...
    print("=" * 75)
    print(" MAIZE PEST & DISEASE CLASSIFIER — 7-CLASS PRODUCTION TRAINING ")
    print("=" * 75)
    ...
    print(f"[Maize Training] Computing Device: {device} (threads={torch.get_num_threads()})")
    ...
    if not os.path.exists(train_file) or not os.path.exists(val_file):
        raise FileNotFoundError("Splits not found. Please run ml.src.dataset_maize first.")
    ...
    print(f"[Maize Training] Loaded {len(train_samples)} training samples, {len(val_samples)} validation samples across {num_classes} classes.")

    input_size = cfg["training"]["input_size"]
    batch_size = cfg["training"]["batch_size"]
    warmup_epochs = cfg["training"].get("warmup_epochs", 2)
    finetune_epochs = cfg["training"].get("finetune_epochs", 4)

    train_dataset = MaizeLeafDataset(train_samples, transform=get_training_transforms(input_size))
    val_dataset = MaizeLeafDataset(val_samples, transform=get_validation_transforms(input_size))
    ...
    # Save class_names.json and class_metadata.json
    save_json(class_names, os.path.join(models_dir, "class_names.json"))

    class_metadata = {
        "classes": [
            {"name": c, "category": CLASS_CATEGORIES.get(c, "disease")}
            for c in class_names
        ]
    }
    save_json(class_metadata, os.path.join(models_dir, "class_metadata.json"))

    num_params = sum(p.numel() for p in model.parameters())
    metadata = {
        "model_name": "MaizePestAndDiseaseClassifier-MobileNetV3",
    ...
    save_json(metadata, os.path.join(models_dir, "model_metadata.json"))
