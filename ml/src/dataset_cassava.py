"""
Cassava Dataset Audit, Validation, Stratified Splitting, and PyTorch Dataset Definition.
Covers 5 classes:
  - Disease: Bacterial Blight, Brown Spot, Mosaic
  - Pest: Green Mite (explicitly pest)
  - Healthy: Healthy
...
class CassavaLeafDataset(Dataset):
    """PyTorch Dataset loading Cassava leaf images from manifest file."""
...
def find_cassava_dataset_dir(configured_dir: str = "Cassava") -> str:
    """
    Search and verify Cassava dataset directory automatically without hardcoding assumptions.
    """
    candidates = [
        configured_dir,
        os.path.join(REPO_ROOT, configured_dir),
        os.path.join(REPO_ROOT, "cassava"),
        os.path.join(REPO_ROOT, "Dataset", "Cassava"),
        "C:\\Users\\krsan\\Desktop\\crop\\Cassava",
    ]
    ...
    for cand in candidates:
        if os.path.exists(cand) and os.path.isdir(cand):
            ...
            # Verify required class folders exist
            matches = [
                d for d in subdirs
                if any(k in d for k in ["bacterial", "brown", "mite", "healthy", "mosaic"])
            ]
            if len(matches) >= 4:
    ...
    for root, dirs, _ in os.walk(REPO_ROOT):
        ...
        if "bacterial blight" in lowered and "green mite" in lowered:
            return os.path.abspath(root)

    raise FileNotFoundError(
        f"Could not automatically locate the Cassava dataset folder. Searched candidates: {candidates}"
    )


def audit_cassava_dataset(raw_dir: str, valid_extensions: List[str]) -> Dict:
    """
    Inspect the raw dataset, detect corruptions, quarantine duplicates, and compute statistics.
    """
    print(f"\n[Audit] Inspecting Cassava dataset at: {raw_dir}")
    subdirs = [
    ...
    report = {
        "dataset_name": "Cassava Leaf Disease & Pest Dataset",
...
def main():
    config_path = os.path.join(REPO_ROOT, "ml", "config_cassava.yaml")
    config = load_config(config_path)

    raw_dir = find_cassava_dataset_dir(config["dataset"]["raw_dir"])
    valid_exts = config["dataset"]["image_extensions"]

    report, valid_samples = audit_cassava_dataset(raw_dir, valid_exts)
    ...
    print("\n" + "=" * 55)
    print("           CASSAVA DATASET REPORT")
    print("=" * 55)
    for c in CANONICAL_CASSAVA_CLASSES:
