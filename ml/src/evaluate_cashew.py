"""
Comprehensive Evaluation of Trained 5-Class Cashew Disease & Pest Classifier on Independent Test Set.
...
from ml.src.dataset_cashew import CashewLeafDataset, CANONICAL_CASHEW_CLASSES
from ml.src.augmentation import get_validation_transforms
from ml.src.model import build_model


def evaluate_cashew_test_set(
    config_path: str = "ml/config_cashew.yaml",
    model_weights_path: str = None,
    batch_size: int = 32,
) -> dict:
    print("=" * 75)
    print(" EVALUATION ON INDEPENDENT CASHEW TEST SPLIT (5 CLASSES) ")
    print("=" * 75)
    ...
    print(f"[Cashew Evaluation] Total independent test samples: {len(test_samples)}")

    test_dataset = CashewLeafDataset(
        test_samples,
    ...
    model.eval()
    print(f"[Cashew Evaluation] Loaded checkpoint from: {model_weights_path}")
    ...
    print("\n" + "=" * 65)
    print("        CASHEW HELD-OUT TEST EVALUATION METRICS")
    print("=" * 65)
