"""
Production Training Pipeline for 5-Class Cassava Leaf Disease & Pest Classification:
...
from ml.src.dataset_cassava import CassavaLeafDataset, CANONICAL_CASSAVA_CLASSES, CLASS_CATEGORIES
...
def compute_class_weights(train_samples: List[Dict], num_classes: int) -> torch.Tensor:
    ...
    print(f"[Cassava Training] Class counts in training set: {counts}")
    print(f"[Cassava Training] Computed class weights: {[round(w, 3) for w in weights_tensor.tolist()]}")
...
def train_cassava_model():
    ...
    train_dataset = CassavaLeafDataset(train_samples, class_to_idx, transform=train_transform)
    val_dataset = CassavaLeafDataset(val_samples, class_to_idx, transform=val_transform)
    ...
    print(f"[Cassava Training] Target Device: {device}")
    print(f"[Cassava Training] Training Batches: {len(train_loader)} | Validation Batches: {len(val_loader)}")
    ...
    print("\n" + "=" * 65)
    print(f"[Cassava Training Complete] Total Time: {total_time/60:.2f} min | Best Val Acc: {best_val_acc:.2f}% (Epoch {best_epoch})")
    print("=" * 65)
