...
export const SpreadRiskGraphViewer: React.FC<SpreadRiskGraphViewerProps> = ({
  ...
  const getTierBadge = (tier: string) => {
    switch (tier) {
      ...
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/40 text-emerald-400 text-[10px] font-bold">
            LOW THREAT
          </span>
        );
    }
    ...
    <div className="p-5 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-5 shadow-xl">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-2">
          <div className="px-3 py-1 rounded-xl bg-slate-950 border border-slate-800 flex items-center gap-2 text-xs font-mono text-slate-300">
            <Wind className="w-3.5 h-3.5 text-blue-400" />
            <span>Wind: {graphData?.wind_parameters.speed_kmh} km/h (SW @ {graphData?.wind_parameters.direction_degrees}°)</span>
          </div>

          <div className="px-2.5 py-1 rounded-xl bg-slate-950 border border-slate-800 flex items-center gap-1.5 text-[10px] text-slate-400 font-medium">
                ...
                <div
                  ...
                  className={`p-4 rounded-2xl bg-slate-950 border transition cursor-pointer space-y-3 ${
                    selectedEdge?.source_farm_id === edge.source_farm_id
                      ? 'border-emerald-500/80 bg-slate-950/90 shadow-md shadow-emerald-950/30'
                      : 'border-slate-800 hover:border-slate-700'
                  }`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-bold text-slate-100 text-xs">{edge.source_farm_name}</p>
                      <p className="text-[10px] text-slate-400 font-mono">{edge.distance_km.toFixed(1)} km away</p>
                    </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800/80 space-y-1 text-xs">
                    <div className="flex justify-between">
                      <span className="text-slate-400 text-[11px]">Estimated Spread Risk:</span>
                      <span className="font-bold font-mono text-rose-400">{edge.estimated_spread_risk} / 100</span>
                    </div>

                    <div className="flex justify-between">
                      <span className="text-slate-400 text-[11px]">Active Pathogen:</span>
                      <span className="text-slate-200 text-[10px] truncate max-w-[130px] font-semibold">
                    ...
                    {edge.estimated_arrival_days && (
                      <div className="flex justify-between">
                        <span className="text-slate-400 text-[11px]">Est. Arrival Window:</span>
            ...
            {selectedEdge && (
              <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 text-xs space-y-2">
                <div className="flex items-center justify-between text-slate-400 border-b border-slate-800 pb-2">
                  <span className="font-semibold text-slate-200">Transmission Corridor Rationale</span>
                ...
                <p className="text-slate-300 leading-relaxed text-[11px]">{selectedEdge.explanation}</p>
              </div>
            )}
          </div>
            ...
            <div className="space-y-2.5">
              {hotspots.map((hs) => (
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <span className="font-bold text-slate-200 block text-[11px]">{hs.name}</span>
                      <span className="text-[10px] text-slate-400 font-mono">Radius: {hs.radius_km} km</span>
                    </div>
