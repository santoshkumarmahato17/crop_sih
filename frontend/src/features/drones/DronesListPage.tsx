...
export const DronesListPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-agri-900 border border-slate-800 shadow-xl">
        <div>
          <h1 className="text-2xl font-extrabold text-agri-100 flex items-center gap-2">
            <Cpu className="w-6 h-6 text-agri-400" />
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-agri-500 hover:bg-agri-500 text-white text-xs font-extrabold shadow-lg shadow-emerald-950/40 transition hover:scale-[1.02]"
        >
      ...
      {/* Fleet Overview Grid */}
      {isLoading ? (
        <div className="p-12 text-center text-agri-400/70">Loading drone fleet...</div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <h3 className="font-bold text-agri-100 text-base">{drone.name}</h3>
                    <p className="text-xs text-agri-400/70 mt-0.5">
                      {drone.manufacturer} • {drone.model_name}
                    </p>
                  </div>

                  <span className="px-2.5 py-0.5 rounded-full bg-agri-500/10 border border-agri-500/25 text-agri-400 text-xs font-semibold uppercase">
                    {drone.operational_status}
                  </span>
                </div>

                <div className="p-3 rounded-xl bg-agri-950/60 border border-slate-800/80 space-y-2 text-xs">
                  <div className="flex items-center justify-between text-agri-400/70">
                    <span className="flex items-center gap-1.5">
                      <Battery className="w-3.5 h-3.5 text-agri-400" />
                      <span>Battery Status</span>
                    </span>
                    <span className="font-mono font-bold text-agri-200">
                ...
                <div className="space-y-1.5 text-xs">
                  <span className="text-agri-400/70 flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {(drone.sensor_capabilities || ['RGB', 'NIR']).map((band, idx) => (
                      <span
                        key={idx}
                        className="px-2 py-0.5 rounded-md bg-agri-800 text-agri-300 text-[10px] font-mono"
                      >
                        {band}
                      </span>
                    ))}
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-between text-xs text-agri-500/70 font-mono">
                <span>S/N: {drone.serial_number}</span>
                <span className="text-agri-400 flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-50 bg-agri-950/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="max-w-md w-full p-6 rounded-2xl bg-agri-900 border border-slate-800 shadow-2xl space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-agri-100 text-base flex items-center gap-2">
            ...
            <form onSubmit={handleRegister} className="space-y-4 text-xs">
              <div>
                <label className="block font-medium text-agri-300 mb-1">Drone Name *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-agri-100 focus:outline-none focus:border-agri-500"
                />
              </div>

              <div>
                <label className="block font-medium text-agri-300 mb-1">Hardware Serial Number *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block font-medium text-agri-300 mb-1">Manufacturer</label>
                  <input
                    type="text"
                    value={manufacturer}
                ...
                <div>
                  <label className="block font-medium text-agri-300 mb-1">Model Name</label>
                  <input
                    type="text"
                    value={modelName}
              ...
              <div>
                <label className="block font-medium text-agri-300 mb-1">Camera & Sensor Payload</label>
                <input
                  type="text"
                  value={cameraType}
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-800">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 rounded-xl bg-agri-800 text-agri-300 font-medium"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-agri-500 hover:bg-agri-500 text-white font-bold transition flex items-center gap-1.5 disabled:opacity-50"
                >
