...
export const MissionsListPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      default:
        return (
          <span className="px-2.5 py-0.5 rounded-full bg-slate-800 text-slate-400 text-xs">
            {status}
          </span>
        );
    }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Mission Cards Grid */}
      {isLoading ? (
        <div className="p-12 text-center text-slate-400">Loading flight missions...</div>
          ...
          {filteredMissions.map((m) => (
            ...
            >
              <div className="space-y-3">
                ...
                <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800/80 grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-slate-500 block">Flight Date:</span>
