...
export const Step7AIAnalysis: React.FC<Props> = ({ onPrev, onReset }) => {
  ...
  const runAnalysis = async () => {
    ...
    const payload: SymptomAnalysisPayload = {
      ...
      crop_type: selectedCrop || 'Not identified',
  ...
  const isSimulated = (analysisResult as any).is_simulated ?? true;
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
        ...
        <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-extrabold text-xs border ${
          ...
          {isSimulated ? 'Demo Mode - Simulated Output' : 'AI Suspected'}
        </span>
      </div>
        ...
        <div className="p-6 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 space-y-4">
          ...
          <div className="space-y-2 pt-2">
            <div className="flex justify-between text-xs font-bold text-slate-700 dark:text-slate-300">
              <span>{isSimulated ? 'Simulated Confidence (Demo)' : 'Model Confidence'}</span>
