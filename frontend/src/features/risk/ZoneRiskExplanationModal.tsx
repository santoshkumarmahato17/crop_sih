...
export const ZoneRiskExplanationModal: React.FC<ZoneRiskExplanationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="max-w-2xl w-full p-6 rounded-2xl bg-agri-900 border border-slate-800 shadow-2xl space-y-5 max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        {isLoading ? (
          ...
          <div className="space-y-5">
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
                <div className="space-y-0.5">
                  <span className="text-[11px] text-agri-400/70 font-medium block">Overall Crop Risk Score</span>
                  <div className="flex items-center gap-3">
                    <span className="text-3xl font-black font-mono text-rose-400">
                      {assess.overall_crop_risk}
                      <span className="text-xs text-agri-500/70 font-normal font-sans"> / 100</span>
                    </span>
                    {getRiskTierBadge(assess.risk_level)}
                  </div>
                </div>

                <span className="px-2.5 py-1 rounded-md bg-agri-900 border border-slate-800 text-[10px] text-agri-400/70 font-mono self-start sm:self-auto">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1">
                  <span className="text-[11px] text-agri-400/70 font-semibold flex items-center justify-center gap-1">
                    <ShieldAlert className="w-3 h-3 text-rose-400" />
                    <span>Disease Risk</span>
                  </span>
                  <p className="text-xl font-bold font-mono text-rose-400">
                    {assess.disease_risk_score} <span className="text-[10px] text-agri-500/70 font-sans">/100</span>
                  </p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1">
                  ...
                  <p className="text-xl font-bold font-mono text-amber-400">
                    {assess.pest_risk_score} <span className="text-[10px] text-agri-500/70 font-sans">/100</span>
                  </p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1">
                  ...
                  <p className="text-xl font-bold font-mono text-blue-400">
                    {assess.water_stress_risk} <span className="text-[10px] text-agri-500/70 font-sans">/100</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
              ...
              <div className="space-y-2">
                {assess.contributing_factors.map((factor, idx) => (
                  ...
                  >
                    <div className="space-y-0.5">
                      <span className="font-bold text-agri-200 block">{factor.factor_name}</span>
                      <p className="text-[11px] text-agri-400/70 leading-relaxed">{factor.description}</p>
                    </div>
                    ...
                    <span
                      className={`px-2.5 py-1 rounded-lg font-mono font-bold text-xs flex-shrink-0 ${
                        factor.points > 0
                          ...
                          : 'bg-agri-500/15 border border-agri-500/25 text-agri-400'
                      }`}
                    >
            ...
            {/* Summary Banner */}
            <div className="p-3.5 rounded-xl bg-agri-950/80 border border-slate-800 text-xs text-agri-300 flex items-start gap-2">
