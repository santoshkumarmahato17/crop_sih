...
export const WeatherRiskForecastCard: React.FC<WeatherRiskForecastCardProps> = ({
  ...
  const getTierColor = (tier: string) => {
    switch (tier) {
      ...
      default:
        return 'text-agri-600 dark:text-agri-400 bg-agri-500/10 border-agri-500/25';
    }
  };
  ...
  const getTrendIcon = (trend: string) => {
    if (trend.includes('RISING')) {
      ...
      return (
        <span className="flex items-center gap-1 text-agri-600 dark:text-agri-400 text-xs font-bold">
    ...
    return (
      <span className="flex items-center gap-1 text-agri-500/70 dark:text-agri-400/70 text-xs font-bold">
  ...
  return (
    <div className="rounded-3xl bg-white dark:bg-surface-darkCard/90 border border-agri-200/50 dark:border-agri-700/25 shadow-xl overflow-hidden space-y-6 p-6 transition">
      {/* ── 1. Top Header: Weather Telemetry & Trend ── */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-agri-100 dark:border-agri-700/25 pb-5">
        <div className="space-y-1">
          ...
          <h2 className="text-xl font-black text-agri-900 dark:text-white tracking-tight">
            Microclimate Telemetry & Predictive Epidemic Risk
          </h2>
          <p className="text-xs text-agri-500/70 dark:text-agri-400/70">
            Current Farm: <strong className="text-agri-800 dark:text-agri-200">{riskData.farm_name}</strong> • Updated {new Date(riskData.evaluated_at).toLocaleTimeString()}
          </p>
        </div>

        {/* Horizon Picker & Refresh */}
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-agri-50 dark:bg-agri-800/50 p-1 rounded-2xl border border-agri-200/50 dark:border-agri-700/30">
            {[3, 5, 7].map((days) => (
              <button
                key={days}
                type="button"
                ...
                className={`px-3 py-1 rounded-xl text-xs font-bold transition ${
                  selectedHorizon === days
                    ...
                    : 'text-agri-600 dark:text-agri-300 hover:text-agri-900 dark:hover:text-white'
                }`}
              >
                {days} Days
              </button>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 p-4 rounded-2xl bg-surface-light dark:bg-agri-950/70 border border-agri-200/50 dark:border-agri-700/25">
        <div className="flex items-center gap-2.5">
          <div className="p-2 rounded-xl bg-amber-500/10 text-amber-600 dark:text-amber-400">
            <Thermometer className="w-4 h-4" />
          </div>
          <div>
            <span className="text-[10px] text-agri-500/70 block font-semibold">Temperature</span>
            <span className="text-sm font-black font-mono text-agri-800 dark:text-agri-100">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-[10px] text-agri-500/70 block font-semibold">Humidity</span>
            <span className="text-sm font-black font-mono text-agri-800 dark:text-agri-100">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-[10px] text-agri-500/70 block font-semibold">Rainfall</span>
            <span className="text-sm font-black font-mono text-agri-800 dark:text-agri-100">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <span className="text-[10px] text-agri-500/70 block font-semibold">Wind Speed</span>
            <span className="text-sm font-black font-mono text-agri-800 dark:text-agri-100">
        ...
        <div className="flex items-center gap-2.5 col-span-2 sm:col-span-1">
          ...
          <div>
            <span className="text-[10px] text-agri-500/70 block font-semibold">Microclimate</span>
            <span className="text-xs font-bold text-agri-800 dark:text-agri-100 truncate block">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {/* Disease Risk */}
        <div className="p-4 rounded-2xl bg-white dark:bg-surface-darkCard border border-agri-200/50 dark:border-agri-700/25 shadow-sm space-y-3 flex flex-col justify-between">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-agri-500/70 dark:text-agri-400/70 flex items-center gap-1.5">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-agri-400/70 font-mono">/ 100</span>
            </div>
          ...
          <div className="pt-2 border-t border-agri-100 dark:border-agri-700/25 flex items-center justify-between text-[10px] text-agri-400/70 font-mono">
            <span>Forecast Confidence:</span>
            <strong className="text-agri-700 dark:text-agri-300 font-bold">{curDisease.confidence_pct}%</strong>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-surface-darkCard border border-agri-200/50 dark:border-agri-700/25 shadow-sm space-y-3 flex flex-col justify-between">
          ...
          <div className="pt-2 border-t border-agri-100 dark:border-agri-700/25 flex items-center justify-between text-[10px] text-agri-400/70 font-mono">
            <span>Forecast Confidence:</span>
            <strong className="text-agri-700 dark:text-agri-300 font-bold">{curPest.confidence_pct}%</strong>
          </div>
        </div>
        ...
        <div className="p-4 rounded-2xl bg-white dark:bg-surface-darkCard border border-agri-200/50 dark:border-agri-700/25 shadow-sm space-y-3 flex flex-col justify-between">
          ...
          <div className="pt-2 border-t border-agri-100 dark:border-agri-700/25 flex items-center justify-between text-[10px] text-agri-400/70 font-mono">
            <span>Forecast Confidence:</span>
            <strong className="text-agri-700 dark:text-agri-300 font-bold">{curWater.confidence_pct}%</strong>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-black text-agri-900 dark:text-white flex items-center gap-2">
            <Calendar className="w-4 h-4 text-sky-500" />
            <span>Forward Risk Forecast Timeline ({selectedHorizon} Days)</span>
          </h3>
          <span className="text-xs text-agri-400/70">Click a day to inspect forecast</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className="text-[11px] font-black text-agri-800 dark:text-agri-200">{dayName}</span>
                  <span className={`px-1.5 py-0.2 rounded text-[9px] font-bold ${getTierColor(item.overall_tier)}`}>
                    {item.overall_tier.slice(0, 4)}
                  </span>
                </div>

                <div className="text-xs font-mono font-bold text-agri-600 dark:text-agri-400/70">
                ...
                <span className="text-[10px] font-mono text-agri-500/70 font-bold">
        ...
        <div className="p-4 rounded-2xl bg-sky-500/5 border border-sky-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
          <div className="space-y-1">
            <span className="font-black text-agri-900 dark:text-white flex items-center gap-1.5">
            ...
            <p className="text-agri-600 dark:text-agri-300 font-medium">
      ...
      <div className="p-4 rounded-2xl bg-agri-500/10 border border-agri-500/25 flex items-start gap-3">
        <Sparkles className="w-5 h-5 text-agri-600 dark:text-agri-400 flex-shrink-0 mt-0.5" />
        <div className="space-y-1">
          <h4 className="text-xs font-black uppercase tracking-wider text-agri-700 dark:text-agri-400">
      ...
      <div className="border-t border-agri-100 dark:border-agri-700/25 pt-3">
        <button
          type="button"
          onClick={() => setShowTechnicalDetails(!showTechnicalDetails)}
          className="w-full flex items-center justify-between text-xs text-agri-500/70 dark:text-agri-400/70 hover:text-agri-800 dark:hover:text-agri-200 font-bold py-1 transition"
        >
        ...
        {showTechnicalDetails && (
          <div className="mt-4 p-4 rounded-2xl bg-surface-light dark:bg-surface-darkBg border border-agri-200/50 dark:border-agri-700/25 space-y-4 animate-in fade-in">
            <div className="space-y-2">
              <h5 className="text-xs font-black text-agri-800 dark:text-agri-200 uppercase tracking-wider">
              ...
              <div className="space-y-2">
                {curDisease.contributing_factors?.map((f, i) => (
                  <div
                    key={i}
                    className="p-2.5 rounded-xl bg-white dark:bg-surface-darkCard border border-agri-200/50 dark:border-agri-700/25 flex items-start justify-between gap-3 text-xs"
                  >
                    <div>
                      <strong className="text-agri-800 dark:text-agri-200 block font-bold">{f.factor_name}</strong>
                      <span className="text-agri-500/70 text-[11px]">{f.description}</span>
                    </div>
            ...
            <div className="text-[11px] text-agri-400/70 font-mono space-y-1 pt-2 border-t border-agri-200/50 dark:border-agri-700/25">
