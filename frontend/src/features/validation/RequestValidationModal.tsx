  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-agri-950/80 backdrop-blur-md overflow-y-auto animate-in fade-in">
      <div className="relative w-full max-w-lg bg-white dark:bg-surface-darkCard border border-agri-200/50 dark:border-agri-700/25 rounded-3xl shadow-2xl overflow-hidden my-8">
        {/* Header */}
        <div className="p-6 border-b border-agri-100 dark:border-agri-700/25 bg-gradient-to-r from-purple-900 to-slate-900 text-white flex items-center justify-between">
        ...
        {isSubmitted ? (
          <div className="p-8 text-center space-y-4">
            ...
            <p className="text-xs text-agri-600 dark:text-agri-300 max-w-xs mx-auto">
              Case <strong>{caseNumber}</strong> has been created and assigned to the Regional Agriculture Extension Officer.
            </p>
            <button
              type="button"
              onClick={onSuccess}
              className="w-full py-3 rounded-2xl bg-agri-500 hover:bg-emerald-700 text-white font-bold text-xs transition shadow-lg shadow-emerald-900/20"
            >
              Back to Dashboard
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            {/* Condition badge */}
            <div className="p-3.5 rounded-2xl bg-surface-light dark:bg-surface-darkBg border border-agri-200/50 dark:border-agri-700/25 flex items-center justify-between text-xs">
              <div>
                <span className="text-agri-500/70 font-bold block">Target Condition</span>
                <span className="font-extrabold text-agri-900 dark:text-white">
                  {advisory?.condition_name || 'Suspected Crop Disease'}
                </span>
              </div>
              <div className="text-right">
                <span className="text-agri-500/70 font-bold block">Location</span>
                <span className="font-semibold text-agri-700 dark:text-agri-300">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-surface-light dark:bg-surface-darkBg border border-agri-200/50 dark:border-agri-700/25 text-xs font-bold focus:outline-none focus:ring-2 focus:ring-purple-500"
              >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-surface-light dark:bg-surface-darkBg border border-agri-200/50 dark:border-agri-700/25 text-xs focus:outline-none focus:ring-2 focus:ring-purple-500"
                required
              />
            </div>
