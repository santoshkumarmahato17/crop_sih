...
export const Sidebar: React.FC<SidebarProps> = ({ onClose, isMobileDrawer = false }) => {
  ...
  const farmerNavItems: NavItem[] = [
    ...
    { to: '/farms/new', label: 'Add Farm', icon: Plus, iconColor: 'text-[#a855f7]' },
    { to: '/farms', label: 'My Farm', icon: ClipboardList, iconColor: 'text-[#eab308]' },
  ...
  return (
    <aside
      className="w-72 max-w-full border-r border-[#D4E8DF] dark:border-[#16383B] bg-white dark:bg-[#0D2729] text-[#12302E] dark:text-[#E6F5F0] flex flex-col justify-between p-4 overflow-y-auto shrink-0 shadow-2xl transition-colors duration-200 h-full"
    >
      <div className="space-y-3">
        {/* Mobile Close Button (Only visible in drawer mode) */}
        {onClose && (
          <div className="flex justify-end px-1 pt-0.5 pb-2.5 border-b border-[#D4E8DF] dark:border-[#16383B]">
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-xl bg-slate-100 dark:bg-[#071A1D] hover:bg-slate-200 dark:hover:bg-[#16383B] text-slate-600 dark:text-[#E6F5F0] hover:text-slate-900 dark:hover:text-white border border-[#D4E8DF] dark:border-[#16383B] transition"
        ...
        <div className="flex items-center justify-between px-2 pt-1 pb-1">
          <span className="text-[10px] font-bold uppercase tracking-wider text-[#5F7775] dark:text-[#8CAFA8]">
            {roleLabel}
          </span>
          <span
            className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded-full border ${
              role === 'ADMIN'
                ? 'bg-purple-100 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-800/40'
                : role === 'GOVERNMENT'
                ? 'bg-sky-100 dark:bg-sky-950/40 text-sky-600 dark:text-sky-400 border-sky-200 dark:border-sky-800/40'
                : 'bg-[#E8F5EF] dark:bg-[#0F3532] text-[#087B62] dark:text-[#2FA36B] border-[#D4E8DF] dark:border-[#16383B]'
            }`}
          >
            {role.replace('_', ' ')}
          </span>
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center justify-between px-3 py-2.5 rounded-xl text-[12.5px] font-medium transition-all duration-150 group ${
                    isActive
                      ? 'bg-[#E8F5EF] dark:bg-[#0F3532] border border-[#087B62]/30 dark:border-[#2FA36B]/40 text-[#087B62] dark:text-[#2FA36B] font-semibold shadow-sm'
                      : 'text-[#12302E] dark:text-[#E6F5F0] hover:bg-slate-100 dark:hover:bg-[#071A1D] hover:text-[#087B62] dark:hover:text-[#2FA36B] border border-transparent'
                  }`
                }
              >
                <div className="flex items-center gap-3">
                  <Icon
                    className={`w-[17px] h-[17px] flex-shrink-0 transition-transform duration-150 group-hover:scale-110 ${
                      item.iconColor || 'text-[#087B62] dark:text-[#2FA36B]'
                    }`}
                  />
