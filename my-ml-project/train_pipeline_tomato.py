...
def extract_features(csv_path, embeddings_path, labels_path, folds_path):
    ...
    joblib.dump(le, 'tomato_label_encoder.pkl')
...
# ==========================================
# 2. Model Training Functions (Optimized for Tomato without SMOTE)
# ==========================================

def train_lightgbm_tuned(X_train, y_train, X_val, y_val, class_weights=None):
    clf = lgb.LGBMClassifier(n_estimators=200, learning_rate=0.03, max_depth=8, class_weight='balanced', random_state=42, n_jobs=-1, verbose=-1)
...
def train_catboost_tuned(X_train, y_train, X_val, y_val, class_weights=None):
    # Idea 3: More robust defaults for CatBoost to let it learn dense features naturally
    clf = CatBoostClassifier(iterations=200, learning_rate=0.08, depth=4, auto_class_weights='Balanced', random_state=42, verbose=0, thread_count=-1)
...
class AdvancedMLP(nn.Module):
    # Idea 2: Deeper architecture with high dropout to prevent collapse
...
def train_pytorch_mlp(X_train, y_train, X_val, y_val, num_classes, class_weights=None):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = AdvancedMLP(input_dim=X_train.shape[1], num_classes=num_classes).to(device)
...
def evaluate_model_kfold(model_func, model_name, features, labels, folds, num_classes):
    ...
    for k in range(5):
        ...
        # Idea 1: We removed SMOTE completely. It destroyed performance on the dense Tomato feature space.
...
def run_pipeline():
    csv_path = 'tomato_dataset_folds.csv'
    features_path = 'tomato_embeddings.npy'
    labels_path = 'tomato_labels.npy'
    folds_path = 'tomato_folds.npy'
    ...
    save_path = f"best_tomato_model_{best_model_info['name'].lower()}{best_model_info['extension']}"
