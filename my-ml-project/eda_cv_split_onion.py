...
def perform_eda_and_cv(dataset_path, output_dir):
    ...
    plt.title('Class Distribution for Onion Dataset')
    plt.xlabel('Target Class')
    plt.ylabel('Number of Images')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plot_path = os.path.join(output_dir, "onion_class_distribution.png")
    ...
    output_csv = os.path.join(output_dir, "onion_dataset_folds.csv")
    df.to_csv(output_csv, index=False)
    print(f"\nSaved dataset split definitions to {os.path.abspath(output_csv)}")

if __name__ == "__main__":
    base_dir = r"C:\Users\krsan\Desktop\crop"
    potential_names = ["dataset onion\\onion datasets", "dataset Onion\\onion datasets", "Onion\\onion datasets"]
    dataset_path = None
    ...
    if not dataset_path:
        print(f"Dataset path could not be found in {base_dir}")
    else:
